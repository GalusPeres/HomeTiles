#include "src/devices/waveshare_4b/local_camera_board.h"

#if defined(DEVICE_WAVESHARE_4B) && defined(HOMETILES_LOCAL_CAMERA)
...
local_camera::BoardError acquire(i2c_master_bus_handle_t* sccb_bus) {
  ...
  // The bus exists only after init() ran init_display_power(), whose MIPI
  // PHY power cycle needs the only reference on LDO channel 3. Checking the
  // bus first keeps this reference out of that cycle.
  i2c_master_bus_handle_t bus = DeviceWaveshare4B::sharedI2cBus();
...
#endif  // defined(DEVICE_WAVESHARE_4B) && defined(HOMETILES_LOCAL_CAMERA)
