...
inline const lv_font_t* value_font() { return tile_layout::content_font_24(); }
inline int header_height() { return (GRID_CELL_H - GRID_GAP) / 2; }
inline void apply(lv_obj_t* card, lv_obj_t* icon, lv_obj_t* title, lv_obj_t* value, const Tile& tile, const lv_font_t* selected_value_font = nullptr) {
  ...
  const int width = tile_geometry::extent(tile.col, tile.span_w, GRID_CELL_W, GRID_GAP);
  if (icon) {
  ...
  const lv_font_t* value_face = selected_value_font ? selected_value_font : value_font();
