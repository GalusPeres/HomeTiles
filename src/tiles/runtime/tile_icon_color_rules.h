...
// family, Binary sensor or Energy tile: the first matching color rule, else
// the tile's fixed icon color, else `fallback`, the type's own color for this
// state. Unavailable or unknown states (`known` false) always use
// `fallback`. The color goes through tile_icon_disc::set_icon_color(), so the
// disc glow follows; unchanged colors are skipped and nothing is allocated.
