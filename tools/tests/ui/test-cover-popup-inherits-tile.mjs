...
assert.match(opener,
  /init\.bg_color = tile_icon_source::popup_background\(\s*static_cast<lv_obj_t\*>\(lv_event_get_current_target\(event\)\),\s*tileBgColorOrDefault\(\*tile, tileDefaultBgColor\(\)\)\);\s*finish_press_before_popup\(event\);\s*show_cover_popup\(init\);/,
