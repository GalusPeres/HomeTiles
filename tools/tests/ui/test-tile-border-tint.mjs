...
// color: white 20 % normally, the icon hue (40 %, scaled for dark tiles) when
...
assert.ok(disc.includes('inline constexpr lv_opa_t kGlowBorderOpa = 102;'));
assert.match(disc, /if \(tinted\) \{\s*ui_surface_style::set_tile_border_tint\(lv_obj_get_parent\(disc\), color,\s*scaled_opa\(kGlowBorderOpa, step\)\);\s*\} else \{\s*ui_surface_style::clear_tile_border_tint\(lv_obj_get_parent\(disc\)\);/);
...
assert.ok(layout.includes('constexpr int kPopupBorderGlowOpa = 102;'));
...
assert.ok(preview.includes('(scaled(102) / 255).toFixed(3)'));
